...
function App() {
	...
	this.renderCurrent = function () {
		...
		if (!size) size = parseFloat(settings.size);
		if (!size) size = 100;
		let fontIndex = this.selectFamily.selectedIndex,
			variantIndex = this.selectVariant.selectedIndex,
			text = settings.text,
			union = false,
			separate = true,
			bezierAccuracy = "";
	...
	this.loadVariants = function () {
		...
		f.variants.forEach((v) => {
			return this.addOption(this.selectVariant, v);
		});
		this.renderCurrent();
	};
