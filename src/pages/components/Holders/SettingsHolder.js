		...
		<div className='settings'>
			<div className='text-holder'>
				<div className='input-group text-center '>
					<input className='input-text text-center ' type='text' id='input-text' value={inputText} placeholder='Input Text Here' onChange={(e) => setInputText(e.target.value)} />
				</div>
			</div>
			...
			<div className='tuners-holder'>
				...
				<div>
					<div className='input-group'>
						<label>Size:</label>
						<input type='number' id='input-size' value={size} placeholder={100} onChange={(e) => update(SET_SIZE, e.target.value)} />
					</div>

					<div className='input-group'>
						<label>Stroke Width(px):</label>
						<input type='number' id='input-width' value={strokeWidth} placeholder={2 + "px"} onChange={(e) => update(SET_STROKE_WIDTH, e.target.valueAsNumber)} />
					</div>
				</div>
				<div>
					<div className='input-group'>
						<label>Delay(s):</label>
						<input type='number' id='input-delay' placeholder={0.1 + "s"} value={delay} onChange={(e) => update(SET_DELAY, e.target.valueAsNumber)} />
					</div>
					<div className='input-group'>
						<label>Animation duration(s):</label>
						<input type='number' id='input-duration' placeholder='auto' value={duration} onChange={(e) => update(SET_DURATION, e.target.valueAsNumber)} />
					</div>
					<div className='input-group '>
						<label className='inline'>Repeat:</label>
						<input type='checkbox' id='input-duration' placeholder='auto' checked={repeat} onChange={(e) => update(SET_REPEAT, e.target.checked)} />
					</div>
				</div>
				<div>
					<div className='input-group'>
						<label>Timing Function:</label>
						<select id='timing-function' value={timingFunction} onChange={(e) => update(SET_TIMING_FUNCTION, e.target.value)}>
